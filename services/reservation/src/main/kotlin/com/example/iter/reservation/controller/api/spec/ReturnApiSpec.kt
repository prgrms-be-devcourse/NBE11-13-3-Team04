...
interface ReturnApiSpec {
    ...
    @Operation(summary = "반납 확인 대상 목록 조회", description = "로그인 사용자가 등록자인 거래 중 반송이 도착해 최종 확인이 필요한 RETURNED 거래를 조회합니다.")
    @ApiResponses(
        value = [
    ...
    @Operation(summary = "수령·반납 증빙 비교", description = "거래 당사자가 수령 당시와 반납 당시의 상품 상태, 설명, 사진 및 기록 시각을 비교 조회합니다.")
    @ApiResponses(
        value = [
