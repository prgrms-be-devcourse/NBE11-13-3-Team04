package iter.device.controller.admin.spec

import iter.common.dto.response.CursorPageResponse
import iter.common.security.CustomUserDetails
import iter.device.dto.request.AdminEquipmentSearchRequest
import iter.device.dto.request.AdminEquipmentStatusRequest
import iter.device.dto.response.AdminEquipmentDetailResponse
import iter.device.dto.response.AdminEquipmentSummaryResponse
...
interface AdminEquipmentApiSpec {

    @Operation(
        summary = "장비 목록 조회",
        description = "장비명·카테고리·장비 상태 조건으로 삭제된 장비를 포함한 전체 장비를 최신 등록순으로 커서 조회합니다. " +
            "첫 요청에서는 cursor를 생략하고 다음 요청에는 응답의 nextCursor를 전달합니다.",
    )
    @ApiResponses(
        value = [
    ...
    fun getEquipments(
        @Valid @ParameterObject request: AdminEquipmentSearchRequest,
    ...
    @Operation(
        summary = "장비 상태 변경",
        description = "ACTIVE 또는 INACTIVE 장비를 SUSPENDED로 차단합니다. 차단 해제 시에는 장비를 INACTIVE로 복구하며, " +
            "다시 활성화할지는 등록자가 결정합니다. 관리자 처리 이력도 함께 기록합니다.",
    )
    @ApiResponses(
        value = [
