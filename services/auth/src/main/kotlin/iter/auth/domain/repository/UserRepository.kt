...
interface UserRepository : JpaRepository<User, Long> {
    ...
    @Query(
        """
        select new iter.auth.api.UserSummary(u.id, u.nickname)
        from User u
        where u.id = :id
        """,
    )
    fun findSummaryById(@Param("id") id: Long): Optional<UserSummary>
    ...
    fun findSummariesByIdIn(@Param("ids") ids: Collection<Long>): List<UserSummary>
    ...
    fun findWithLockById(userId: Long): Optional<User>
}
