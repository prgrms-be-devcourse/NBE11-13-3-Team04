...
interface EquipmentQueryPort {
    ...
    fun findOwnerId(equipmentId: Long): Optional<Long>

    // 장비 한 건. 상태로 거르지 않는다 (삭제된 장비도 그대로 돌려준다) —
    // "삭제됐으면 거절"은 호출부의 정책이고 에러 코드도 호출부마다 다르다.
    fun find(equipmentId: Long): Optional<EquipmentInfo>

    // 여러 건을 한 번에. 못 찾은 ID 는 결과 Map 에서 빠진다.
    // !! 한 건씩 루프로 호출하지 말 것 !! N+1 이 되고 목킹한 테스트는 통과한다.
    fun findAll(equipmentIds: Collection<Long>): Map<Long, EquipmentInfo>
}
