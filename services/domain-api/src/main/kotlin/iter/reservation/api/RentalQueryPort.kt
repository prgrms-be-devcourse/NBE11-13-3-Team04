...
// 질문을 넘기고 판단은 여기 남긴다.
interface RentalQueryPort {

    // 대여 한 건. 상태로 거르지 않는다 (취소·완료 건도 그대로 돌려준다).
    fun find(rentalId: Long): Optional<RentalInfo>

    // 여러 건을 한 번에. 못 찾은 ID 는 결과 Map 에서 빠진다.
    // !! 한 건씩 루프로 호출하지 말 것 !! N+1 이 되고 목킹한 테스트는 통과한다.
    fun findAll(rentalIds: Collection<Long>): Map<Long, RentalInfo>

    // 이 회원의 탈퇴를 막는 대여가 있는가. 빌린 것과 빌려준 것 양쪽을 본다.
    fun hasWithdrawalBlockingRental(userId: Long): Boolean

    // 관리자 회원 상세에 표시할 거래 건수.
    // 호출 세 번을 한 번으로 묶는다 — 호출부가 상태 집합 상수를 들고 있을 이유가 없다.
    fun countUserRentalStats(userId: Long, today: LocalDate): UserRentalStats

    // 이 장비에 분쟁 중인 대여가 있는가.
    fun hasDisputedRental(equipmentId: Long): Boolean

    // 이 장비의 삭제를 막는 대여가 있는가.
    fun hasDeletionBlockingRental(equipmentId: Long): Boolean

    // 이 기간에 겹치는, 장비를 점유하는 대여가 있는가.
    fun hasConflictingOccupyingRental(equipmentId: Long, from: LocalDate, to: LocalDate): Boolean

    // 이 기간 "밖"에 장비를 점유하는 대여가 있는가.
    // 장비의 대여 가능 기간을 줄일 때, 이미 잡힌 예약이 새 기간을 벗어나는지 확인한다.
    fun hasOccupyingRentalOutsidePeriod(equipmentId: Long, from: LocalDate, to: LocalDate): Boolean

    // 장비의 예약 일정. 취소·거절처럼 일정에 표시하지 않는 상태는 reservation 이 걸러서 준다.
    fun findSchedule(equipmentId: Long, from: LocalDate, to: LocalDate): List<RentalScheduleItem>
}
