...
interface UserQueryPort {
    ...
    //
    // userId 는 nullable 로 선언한다 — 구현체(JpaUserQueryAdapter)가 아직 자바라
    // Long 파라미터가 boxed 참조형이다. 코틀린 non-null Long 은 바이트코드에서
    // primitive long 으로 컴파일돼 자바 구현체의 override 시그니처가 깨진다.
    fun findProfile(userId: Long?): Optional<UserProfile>

    // 화면 표시용 회원 요약. 탈퇴·정지 회원도 그대로 돌려준다.
    //
    // userId 는 nullable 로 선언한다(dispute/service/AdminReportService 의 테스트가
    // never() 검증에서 타입 미지정 any() 를 쓴다 — Mockito any()/boxed Long 함정,
    // device D7/reservation R6 문서 참고). 구현은 자바(JpaUserQueryAdapter)라 그대로 둬도
    // 본문을 고칠 필요는 없다.
    fun findSummary(userId: Long?): Optional<UserSummary>
    ...
    fun findSummaries(userIds: Collection<Long>): Map<Long, UserSummary>

    // 이 회원을 신고 대상으로 삼을 수 있는가. 없는 회원과 탈퇴 회원은 둘 다 false.
    //
    // 호출부에 UserStatus 를 노출하지 않으려고 상태를 돌려주지 않는다.
    // 탈퇴 회원을 "없는 회원"과 같게 볼지는 auth 의 판단이다.
    // userId nullable 이유는 findProfile 주석과 같다(구현체가 아직 자바).
    fun isReportable(userId: Long?): Boolean
}
