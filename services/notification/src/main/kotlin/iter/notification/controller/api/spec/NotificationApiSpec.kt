package iter.notification.controller.api.spec
...
interface NotificationApiSpec {
    ...
    fun issueSseTicket(@Parameter(hidden = true) principal: CustomUserDetails): ResponseEntity<SseTicketResponse>

    @Operation(summary = "실시간 알림 구독 (SSE)", description = "쿼리 파라미터로 /sse-ticket에서 발급받은 ticket을 전달한다.")
    fun subscribe(@Parameter(hidden = true) principal: CustomUserDetails): SseEmitter
    ...
    fun getNotifications(
        ...
        @Parameter(description = "읽지 않은 알림만 조회") unreadOnly: Boolean,
    ...
    fun getUnreadCount(@Parameter(hidden = true) principal: CustomUserDetails): ResponseEntity<UnreadCountResponse>

    @Operation(summary = "알림 읽음 처리", security = [SecurityRequirement(name = "JWT")])
    fun markRead(
        @Parameter(hidden = true) principal: CustomUserDetails,
        @Parameter(description = "알림 ID", example = "1", required = true) notificationId: Long,
    ): ResponseEntity<NotificationResponse>

    @Operation(summary = "알림 전체 읽음 처리", security = [SecurityRequirement(name = "JWT")])
    fun markAllRead(@Parameter(hidden = true) principal: CustomUserDetails): ResponseEntity<MarkAllReadResponse>
}
