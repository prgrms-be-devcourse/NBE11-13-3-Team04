...
class ChatWebSocketMessagingIntegrationTest {
    ...
    fun `두 참여자가 서로 보낸 메시지를 순서대로 받고, 재접속해도 중복 없이 이력을 이어받는다`(): Unit = runBlocking {
        ...
        roomParticipantRepository.save(
            com.example.iter.chat.domain.RoomParticipant(roomId = roomId, userId = ownerId, nickname = "등록자"),
        )
        roomParticipantRepository.save(
            com.example.iter.chat.domain.RoomParticipant(roomId = roomId, userId = requesterId, nickname = "요청자"),
        )
