...
//
// 전송 경로: 클라 SEND → (CH6 전까지는 마스킹 없이) DB 저장 → Redis Pub/Sub 발행 →
// RoomBroadcaster가 이 인스턴스에 붙은 같은 방의 모든 세션(보낸 사람 포함)에 전달.
// 보낸 사람도 Pub/Sub을 거쳐서 받는다 — 경로를 하나로 유지해 "내가 보낸 건 즉시 반영,
// 남이 보낸 건 구독으로"처럼 두 갈래로 나누지 않기 위함이다.
@Component
class ChatWebSocketHandler(
    ...
    private suspend fun handleIncoming(
        ...
        val saved = chatMessageWriteService.send(roomId, principal, frame.content)
        val payload = jsonMapper.writeValueAsString(OutgoingFrame.message(saved))
        runCatching { roomBroadcaster.publish(roomId, payload) }
            .onFailure { e -> log.error("chat 메시지 발행 실패 roomId={} messageId={}", roomId, saved.id, e) }
    }
