package iter.admin.payment.controller.spec
...
interface AdminPaymentApiSpec {
    ...
    fun getPayments(@Valid @ParameterObject request: AdminPaymentSearchRequest): ResponseEntity<CursorPageResponse<AdminPaymentSummaryResponse>>

    @Operation(summary = "결제 상세 조회", description = "특정 결제의 상태, 대여자와 예약 당시 장비·대여 정보를 조회합니다.")
    @ApiResponses(
        value = [
    ...
    fun getPayment(
        ...
        @Positive(message = "결제 ID는 1 이상이어야 합니다.")
        paymentId: Long
