...
private const val SSE_SUBSCRIBE_PATH = "/api/v1/notifications/subscribe"
...
class JwtAuthenticationFilter(
    ...
    private val sseTicketResolverProvider: ObjectProvider<SseTicketResolver>,
    ...
    override fun doFilterInternal(
        ...
        if (SSE_SUBSCRIBE_PATH == request.requestURI) {
    ...
    private fun authenticateByBearerToken(request: HttpServletRequest) {
        val token = resolveBearerToken(request) ?: return
        ...
        if (status == TokenStatus.VALID) {
            ...
            log.debug("만료된 토큰으로 요청: {}", request.requestURI)
        }
        // INVALID 토큰은 별도 처리 없이 인증되지 않은 요청으로 흘려보냄 -> 보호된 경로면 401
    }

    private fun authenticateBySseTicket(request: HttpServletRequest) {
        val resolver = sseTicketResolverProvider.ifAvailable ?: return
    ...
    private fun authenticateAs(userId: Long) {
        ...
        val principal = CustomUserDetails.builder().user(user).build()
        val authentication = jwtTokenProvider.getAuthentication(principal)
